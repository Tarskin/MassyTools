...
# Parameters
MASS_MODIFIERS = ['_H','_H','_O']						# Mass modifiers applied to all analytes (Default is addition of H2O)
CHARGE_CARRIER = ['proton']								# The Charge carrier (Default is addition of a proton)
...
S_N_CUTOFF = 6					        				# Minimum signal to noise value of an analyte to be included in the percentage QC

# Calibration rules
CALIB_S_N_CUTOFF = 9			                		# Minimum signal to noise value for a calibrant to be included in the calibration
NUM_LOW_RANGE = 0				        				# Minimum number of calibrants in the first 1/3 of spectrum above specified S/N
NUM_MID_RANGE = 0				        				# Minimum number of calibrants in the second 1/3 of spectrum above specified S/N
...
BLOCKS = {	#######################
			...
			#######################
				...
				##############################
					'aa':{'mass':121.05276384885,
						'carbons':7,
						'hydrogens':7,
						'nitrogens':1,
						'oxygens':1,
						'sulfurs':0},
					'ab':{'mass':120.06874826616,
						'carbons':7,
						'hydrogens':8,
						'nitrogens':2,
						'oxygens':0,
...
#############################################
class Results():
	def __init__(self,name,calibrated,total,totalBck,totalInt,values):
		self.name = name
...
class App():
	def __init__(self,master):
		...
		# FRAME
		frame = Frame(master)
		master.title("MassyTools 0.1.6.3 (RC 5)")
	...
	def infoPopup(self):
		...
		information = ("MassyTools Version 0.1.6.3 (Release Candidate 5)\n\n"
	...
	def outputPopup(self,master):
		...
		self.percSpec.grid(row = 6, column = 0, sticky = W)
		...
		self.perc.grid(row = 7, column = 0, sticky = W)
	...
	def getLocalMaxima(self,data,included):
		...
		for i in included:
			...
			for j in range(0,OUTER_BCK_BORDER):
				windowIntensities = []
			...
			for j in range(0,OUTER_BCK_BORDER-5):
			...
			for line in fr:
				...
				for index,j in enumerate(areas):
					if j > area:
						area = j
						mass = masses[index]
				analytes.append((analyte, mass, area))
		# Get the accurate mass of highest isotope 
		window = 0.25 	# window to search for local maximum
		data = self.readData(self.inputFile)
		inclusion = []
		exactMasses = []
		for i in analytes:
			inclusion.append((float(i[1])-window,float(i[1])+window))
	...
	def extractBackground(self,data,mass,window):
		...
		"""
		cartesianValue = 1000000000000000000000000000000000000000000000000000		# Ridiculous start value
		background = 0
		totals = []
		lowEdge = mass - window
		highEdge = mass + window
		# Get values on the lower m/z side of the analyte
		for i in range(0,OUTER_BCK_BORDER):	
			windowAreas = []
			windowIntensities = []
			begin = self.search_right(data,lowEdge-i*C[0][2],len(data))
			end = self.search_left(data,highEdge-i*C[0][2],len(data))
			if begin == None or end == None:
				print "Specified m/z value outside of spectra range"
		...
		# Get values on the higher m/z side of the analyte
		for i in range(0,OUTER_BCK_BORDER):
			windowAreas = []
			windowIntensities = []
			begin = self.search_right(data,lowEdge+i*C[0][2],len(data))
			...
			for j in data[begin:end]:
				...
				windowIntensities.append(j[1])
			totals.append((windowAreas,windowIntensities))
		# Examine the values from the lower side of the analyte
		for i in range(0,OUTER_BCK_BORDER-5):
			...
			avg = numpy.average(mix)
			# Complex Method
			#if avg * dev < cartesianValue:
				#cartesianValue = avg * dev
			# Simple Method
			if avg < cartesianValue:
				cartesianValue = avg
				backgroundPoint = avg
				backgroundArea = avgBackground
				noise = dev
		# Examine the values from the upper side of the analyte
		for i in range(OUTER_BCK_BORDER,(OUTER_BCK_BORDER*2)-5):
			mix = totals[i][1]+totals[i+1][1]+totals[i+2][1]+totals[i+3][1]+totals[i+4][1]
			avgBackground = numpy.average([sum(totals[i][0]),sum(totals[i+1][0]),sum(totals[i+2][0]),sum(totals[i+3][0]),sum(totals[i+4][0])])
			dev = numpy.std(mix)
			avg = numpy.average(mix)
	...
	def writeResults(self,results):
		...
		with open(outFile,'w') as fw:
			...
			fw.write(name+"\n")
			fw.write("Composition\tMass\tWindow\tTotal\tMaximum SN\t")
			...
			for i in results:
				...
				for j in i.isotopes:
					if j.sn > maxSN:
						maxSN = j.sn
				fw.write(str(i.composition)+"\t"+str(i.mass)+"\t"+str(i.window)+"\t"+str(sum(int(j.obsArea) for j in i.isotopes[1:]))+"\t"+str(maxSN)+"\t")
				for j in i.isotopes:
	...
	def combineResults(self):
		...
		for file in glob.glob(rawFiles):
			...
			with open(file,'r') as fr: 
				...
				totalInt = self.getTotalIntensity(inputfile)
				for i in values:
					total += float(i[3])
					...
					for j in isotopes:
						# Only take elements with a number
						if j[0].isdigit() == 1:
							if float(j) > float(i[5]):
								totalBck += float(j) - float(i[5])
			data.append(Results(name,calibrated,total,totalBck,totalInt,values))
		...
		summaryFile = os.path.join(self.batchFolder,"summary.txt")
		with open(summaryFile,'w') as fw:
				...
				for i in new:
					...
					for j in i.values:
						...
						for k in isotopes:
							# Only take elements with a number
							if k[0].isdigit() == 1:
								if float(k) > float(j[5]):
									value += float(k) - float(j[5])
						fw.write(str(value)+"\t")
					fw.write("\n")
				fw.write("\n")
			...
			if self.relativeIntensity.get() == 1:
				...
				for i in new:
					fw.write(str(i.name)+"\t"+str(i.calibrated)+"\t")
					for j in i.values:
						fw.write(str(float(j[3]) / float(i.total))+"\t")
					...
					for j in i.values:
						...
						for k in isotopes:
							...
							if k[0].isdigit() == 1:
								if float(k) > float(j[5]):
									value += float(k) - float(j[5])
						fw.write(str(float(value) / float(i.totalBck))+"\t")
			...
			if self.percentageBackground.get() == 1:		
				...
				for i in new:
					if i.totalBck == 0:
						...
						for j in i.values:
							if float(j[4]) > S_N_CUTOFF:
								# Get the total intensity after background subtraction
								isotopes = j[8::3]
								for k in isotopes:
								# Only take elements with a number
									if k[0].isdigit() == 1:
										if float(k) > float(j[5]):
											intensity += float(k) - float(j[5])
			...
			if self.isoSignalNoise.get() == 1:
				if numIsotopes >= 1:
					# Compositions
					for i in data:
						fw.write("SN Isotope 1\tCalibrated\t")
						for j in i.values:
							fw.write(str(j[0])+"\t")
						fw.write("\n")
						break
				...
				if numIsotopes >= 2:
					for i in data:
						fw.write("SN Isotope 2\tCalibrated\t")
						for j in i.values:
							fw.write(str(j[0])+"\t")
						fw.write("\n")
						break
				...
				# Isotope 3 Signal to noise block
				if numIsotopes >= 3:
					for i in data:
						fw.write("SN Isotope 3\tCalibrated\t")
						for j in i.values:
							fw.write(str(j[0])+"\t")
						fw.write("\n")
						break
				...
				# Isotope 4 Signal to noise block
				if numIsotopes >= 4:
					for i in data:
						fw.write("SN Isotope 4\tCalibrated\t")
						for j in i.values:
							fw.write(str(j[0])+"\t")
						fw.write("\n")
						break
				...
				# Isotope 5 Signal to noise block
				if numIsotopes >= 5:
					for i in data:
						fw.write("SN Isotope 5\tCalibrated\t")
						for j in i.values:
							fw.write(str(j[0])+"\t")
						fw.write("\n")
						break
				...
				# Isotope 6 Signal to noise block
				if numIsotopes >= 6:
					for i in data:
						fw.write("SN Isotope 6\tCalibrated\t")
						for j in i.values:
							fw.write(str(j[0])+"\t")
						fw.write("\n")
						break
				...
				# Isotope 7 Signal to noise block
				if numIsotopes >= 7:
					for i in data:
						fw.write("SN Isotope 7\tCalibrated\t")
						for j in i.values:
							fw.write(str(j[0])+"\t")
						fw.write("\n")
						break
				...
				# Isotope 8 Signal to noise block
				if numIsotopes >= 8:
					for i in data:
						fw.write("SN Isotope 8\tCalibrated\t")
						for j in i.values:
							fw.write(str(j[0])+"\t")
						fw.write("\n")
						break
				...
				# Isotope 9 Signal to noise block
				if numIsotopes >= 9:
					for i in data:
						fw.write("SN Isotope 9\tCalibrated\t")
						for j in i.values:
							fw.write(str(j[0])+"\t")
						fw.write("\n")
						break
				...
				# Isotope 10 Signal to noise block
				if numIsotopes >= 10:
					for i in data:
						fw.write("SN Isotope 10\tCalibrated\t")
						for j in i.values:
							fw.write(str(j[0])+"\t")
						fw.write("\n")
						break
					for i in new:
						...
						fw.write("\n")
					fw.write("\n")
			# End of Isotopic Signal to Noise Block
			...
			if self.isoAbsoluteIntensity.get() == 1:
				if numIsotopes >= 1:
					# Compositions
					for i in data:
						fw.write("Area Isotope 1 - Background Area\tCalibrated\t")
						for j in i.values:
							fw.write(str(j[0])+"\t")
						fw.write("\n")
						break
					...
					for i in data:
						...
						fw.write("\n")
						break
					# Actual Isotopic AI values				
					for i in new:
						fw.write(str(i.name)+"\t"+str(i.calibrated)+"\t")
						for j in i.values:
							if len(j) >= 11:
								if float(j[8]) > float(j[5]):
									fw.write(str(float(j[8])-float(j[5]))+"\t")
								else:
									fw.write("0\t")
							else:
								fw.write("\t")
				...
				# Isotope 2 Absolute Intensity block (Background subtracted)
				if numIsotopes >= 2:
					for i in data:
						fw.write("Area Isotope 2 - Background Area\tCalibrated\t")
						for j in i.values:
							fw.write(str(j[0])+"\t")
						fw.write("\n")
						break
					for i in new:
						...
						for j in i.values:
							if len(j) >= 14:
								if float(j[11]) > float(j[5]):
									fw.write(str(float(j[11])-float(j[5]))+"\t")
								else:
									fw.write("0\t")
							else:
								fw.write("\t")
				...
				# Isotope 3 Absolute Intensity block (Background subtracted)
				if numIsotopes >= 3:
					for i in data:
						fw.write("Area Isotope 3 - Background Area\tCalibrated\t")
						for j in i.values:
							fw.write(str(j[0])+"\t")
						fw.write("\n")
						break
					for i in new:
						...
						for j in i.values:
							if len(j) >= 17:
								if float(j[14]) > float(j[5]):
									fw.write(str(float(j[14])-float(j[5]))+"\t")
								else:
									fw.write("0\t")
							else:
								fw.write("\t")
				...
				# Isotope 4 Absolute Intensity block (Background subtracted)
				if numIsotopes >= 4:
					for i in data:
						fw.write("Area Isotope 4 - Background Area\tCalibrated\t")
						for j in i.values:
							fw.write(str(j[0])+"\t")
						fw.write("\n")
						break
					for i in new:
						...
						for j in i.values:
							if len(j) >= 20:
								if float(j[17]) > float(j[5]):
									fw.write(str(float(j[17])-float(j[5]))+"\t")
								else:
									fw.write("0\t")
							else:
								fw.write("\t")
				...
				# Isotope 5 Absolute Intensity block (Background subtracted)
				if numIsotopes >= 5:
					for i in data:
						fw.write("Area Isotope 5 - Background Area\tCalibrated\t")
						for j in i.values:
							fw.write(str(j[0])+"\t")
						fw.write("\n")
						break
					for i in new:
						...
						for j in i.values:
							if len(j) >= 23:
								if float(j[20]) > float(j[5]):
									fw.write(str(float(j[20])-float(j[5]))+"\t")
								else:
									fw.write("0\t")
							else:
								fw.write("\t")
				...
				# Isotope 6 Absolute Intensity block (Background subtracted)
				if numIsotopes >= 6:
					for i in data:
						fw.write("Area Isotope 6 - Background Area\tCalibrated\t")
						for j in i.values:
							fw.write(str(j[0])+"\t")
						fw.write("\n")
						break
					for i in new:
						...
						for j in i.values:
							if len(j) >= 26:
								if float(j[23]) > float(j[5]):
									fw.write(str(float(j[23])-float(j[5]))+"\t")
								else:
									fw.write("0\t")
							else:
								fw.write("\t")
				...
				# Isotope 7 Absolute Intensity block (Background subtracted)
				if numIsotopes >= 7:
					for i in data:
						fw.write("Area Isotope 7 - Background Area\tCalibrated\t")
						for j in i.values:
							fw.write(str(j[0])+"\t")
						fw.write("\n")
						break
					for i in new:
						...
						for j in i.values:
							if len(j) >= 29:
								if float(j[26]) > float(j[5]):
									fw.write(str(float(j[26])-float(j[5]))+"\t")
								else:
									fw.write("0\t")
							else:
								fw.write("\t")
				...
				# Isotope 8 Absolute Intensity block (Background subtracted)
				if numIsotopes >= 8:
					for i in data:
						fw.write("Area Isotope 8 - Background Area\tCalibrated\t")
						for j in i.values:
							fw.write(str(j[0])+"\t")
						fw.write("\n")
						break
					for i in new:
						...
						for j in i.values:
							if len(j) >= 32:
								if float(j[29]) > float(j[5]):
									fw.write(str(float(j[29])-float(j[5]))+"\t")
								else:
									fw.write("0\t")
							else:
								fw.write("\t")
				...
				# Isotope 9 Absolute Intensity block (Background subtracted)
				if numIsotopes >= 9:
					for i in data:
						fw.write("Area Isotope 9 - Background Area\tCalibrated\t")
						for j in i.values:
							fw.write(str(j[0])+"\t")
						fw.write("\n")
						break
					for i in new:
						...
						for j in i.values:
							if len(j) >= 35:
								if float(j[32]) > float(j[5]):
									fw.write(str(float(j[32])-float(j[5]))+"\t")
								else:
									fw.write("0\t")
							else:
								fw.write("\t")
				...
				# Isotope 10 Absolute Intensity block (Background subtracted)
				if numIsotopes >= 10:
					for i in data:
						fw.write("Area Isotope 10 - Background Area\tCalibrated\t")
						for j in i.values:
							fw.write(str(j[0])+"\t")
						fw.write("\n")
						break
					for i in new:
						...
						for j in i.values:
							if len(j) >= 38:
								if float(j[35]) > float(j[5]):
									fw.write(str(float(j[35])-float(j[5]))+"\t")
								else:
									fw.write("0\t")
							else:
								fw.write("\t")
						fw.write("\n")
					fw.write("\n")
			# End of Isotopic Absolute Intensity Block (Background subtracted)
	...
	def getTotalIntensity(self, file):
